...
if ($message == "/start" || $message == "/start@itismarconijesibot") {
    ...
    if ($num_rows == 0) {
        ...
        if ($result == 1)
            sendMessage($chat_id, "Benvenuto! Da questo momento iniziarai a ricevere notifiche di nuove circolari, eventi e altre comunicazioni \xF0\x9F\x98\x89");
        else
            sendMessage($chat_id, "Ops...c'è stato un problema nell'avviare il bot \xF0\x9F\x98\x94");
    } else
        sendMessage($chat_id, "Sei già stato aggiunto \xF0\x9F\x98\x89");
    ...
    if (count($tmp) > 1) {
        ...
        if ($numero == 0) {
            ...
            if (mysql_num_rows($result) > 0) {
                ...
                if (count($circolari_keyboard) == 0)
                    sendMessage($chat_id, "Non ho trovato nessuna circolare con nome $cerca \xF0\x9F\x98\x94");
                else {
                    $n_circolari = count($circolari_keyboard);
                    if ($n_circolari == 1)
                        sendKeyboard($chat_id, "Ho trovato una circolare: ", $circolari_keyboard);
                    else
                        sendKeyboard($chat_id, "Ho trovato $n_circolari circolari: ", $circolari_keyboard);
                }
            ...
            if ($num_rows == 0) {
                ...
                if ($num_rows == 1)
                    sendMessage($chat_id, "Ho trovato questa circolare:");
                else
                    sendMessage($chat_id, "Ho trovato queste circolari:");
                while ($row = mysql_fetch_assoc($result)) {
                    sendDocument($chat_id, $row['allegato'], $row['titolo']);
                }
            }
    ...
    if ($tmp[1] == "di") {
        if ($tmp[2] == "ieri") {
            ...
            if (count($circolari_keyboard) == 0)
                sendMessage($chat_id, "Ieri non è uscita nessuna circolare \xF0\x9F\x98\x94");
            else {
                $n_circolari = count($circolari_keyboard);
                if ($n_circolari == 1)
                    sendKeyboard($chat_id, "Ho trovato una circolare: ", $circolari_keyboard);
            ...
            if (count($circolari_keyboard) == 0)
                sendMessage($chat_id, "Oggi non è uscita nessuna circolare \xF0\x9F\x98\x94");
            else {
                $n_circolari = count($circolari_keyboard);
                if ($n_circolari == 1)
                    sendKeyboard($chat_id, "Ho trovato una circolare: ", $circolari_keyboard);
        ...
        if (is_bool($date_circolare)) {
            ...
            if (count($circolari_keyboard) == 0)
                sendMessage($chat_id, "Non c'è nessuna circolare in data $date_circolare_format \xF0\x9F\x98\x94");
            else {
                $n_circolari = count($circolari_keyboard);
                if ($n_circolari == 1)
                    sendKeyboard($chat_id, "Ho trovato una circolare: ", $circolari_keyboard);
        ...
        if ($num_rows == 0) {
            ...
            foreach ($table as $row) {
                if ($row->getAttribute("class") == "jev_evdt_desc") {
                    ...
                    if (strpos(strtolower($nodo), 'circolare') !== false)
                        $circolare_allegata = strtolower($nodo);
                    break;
                }
            }
            ...
            if (!is_null($evento['ora'])) {
                ...
                if (!is_null($comando_circolare))
                    $message .= "\xF0\x9F\x93\x8E	Circolare allegata: $comando_circolare\n";
